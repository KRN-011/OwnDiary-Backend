...
export const expenseCardsAnalyticsService = async ({ userId }) => {
    try {
        ...
        const averageExpenseOfMonthly = await prisma.expense.aggregate({
            where: { userId, createdAt: { gte: new Date(new Date().setMonth(0, 1)) } },
            _avg: { amount: true }
        })

        // Get Average Expense Of Yearly
        const averageExpenseOfYearly = await prisma.expense.aggregate({
            where: { userId, createdAt: { gte: new Date(new Date().setYear(0, 1)) } },
            _avg: { amount: true }
        })
        ...
        const percentageChangeInExpenseCompareToLastMonth = (totalExpenseOfCurrentMonth.amount - totalExpenseOfLastMonth.amount) / totalExpenseOfLastMonth.amount * 100
        ...
        return {
            ...
            data: {
                ...
                percentageChangeInExpenseCompareToLastMonth: percentageChangeInExpenseCompareToLastMonth
            }
        }
    } catch (error) {
        console.log(error)
...
export const expenseGraphAnalyticsService = async ({ userId, startDate, endDate }) => {
    try {
        // Get Expenses
...
export const expenseListAnalyticsService = async ({ userId, startDate, endDate }) => {
    try {

        // Get Top 5 expenses by amount in last 30 days
        const top5ExpensesByAmountInLast30Days = await prisma.expense.findMany({
            where: { userId, createdAt: { gte: new Date(new Date().setDate(new Date().getDate() - 30)) } },
            orderBy: { amount: 'desc' },
            take: 5
        })

        // Get Top 5 expense days by amount in last 30 days
        const top5ExpenseDaysByAmountInLast30Days = await prisma.expense.groupBy({
            by: ['createdAt'],
            where: { userId, createdAt: { gte: new Date(new Date().setDate(new Date().getDate() - 30)) } },
            orderBy: { amount: 'desc' },
            take: 5
